...
public class GraphAdjMatrix extends Graph {
	...
	public List<Integer> getDistance2(int v) {
		...
		// perform matrix multiplication and then iterate through the array
		// for those values that are > 0,

		List<Integer> twoHopVertices = new ArrayList<>();
		int[] resultMatrix = multiplyTwoMatrices2(v, adjMatrix);
		for (int col = 0; col < resultMatrix.length; col++) {
			for (int j = 0; j < resultMatrix[col]; j++) {
				twoHopVertices.add(col);
			}
		}
		
		
		//int[][] resultMatrix = multiplyTwoMatrices(adjMatrix);
		/*for (int col = 0; col < resultMatrix.length; col++) {
			int numPaths = resultMatrix[v][col];
			// only when numPaths is > 0, then we will go into the while loop
	...
	private int[] multiplyTwoMatrices2(int row, int[][] m1) {
		int[] result = new int[m1.length];

		int col = m1.length;

		for (int j = 0; j < col; j++) {
			result[j] = rowColumnMultiplication(m1, row, j);
		}
		return result;
	}

	private int[][] multiplyTwoMatrices(int[][] m1) {
		...
		for (int i = 0; i < row; i++) {
			for (int j = 0; j < col; j++) {
				result[i][j] = rowColumnMultiplication(m1, i, j);
			}
		}
		return result;
	}

	private int rowColumnMultiplication(int[][] matrix, int row, int column) {
		int result = 0;
